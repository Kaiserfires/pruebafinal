...
import { createUserWithEmailAndPassword, getAuth, onAuthStateChanged, sendPasswordResetEmail, signInWithEmailAndPassword, Auth } from '@angular/fire/auth';
...
export class AuthService {
  ...
  async getUserData(id){
    const docSnap: any =await this.apiService.getDocById(`users/${id}`);  //tenes que agregar el apiService
    if (docSnap?.exists()) {
      return docSnap.data();
    }else{
      throw('no existe tal documento');
    }
  }
